...
public class SubdivisionDemo : MonoBehaviour
{
    ...
    Mesh CreatePrimitiveCube(int holeCount = 0) {
        // vertices
    ...
    void SubdivideMeshDemo()
    {
        // recreate parent object
        var parent = GameObject.Find("Subdivs");
        if (parent) Object.DestroyImmediate(parent);
        parent = new GameObject("Subdivs");

        var material = new Material(Shader.Find("Diffuse"));

        // show 4x4 array of subdivided objects
        for (int holes = 0; holes < 4; ++holes)
        {
            ...
            {
                Mesh mesh = MeshData.Subdivide(cube, iterations: iter);
